...
#   --allow-unproduced-context
#                    proceed even when a required context's producer caller is
#                    absent from the target repo (NOT recommended — with
#                    enforce_admins:true every PR waits on the missing check
#                    forever and no --admin escape exists)
#
# --apply REFUSES before any branch-protection PUT when a required context's
# producer is absent from the target repo (the #481 class). The tier's required
# contexts resolve to producers via the canon-derived required-contexts.json
# (generated by install/generate-required-contexts.py, fetched as one file),
# and each producer is read IN THE TARGET REPO via the gh contents API. An
# unreachable target or insufficient token scope also refuses — a guard that
# cannot read reports "cannot verify", never "verified". In the non-mutating
# modes (--check/--dry-run/--report) with --tier, the same finding is REPORTED
# as what --apply would refuse, against the local checkout, without failing.
#
...
#        Also the unproduced-context REFUSAL (nothing mutated — the guard runs
#        before the branch-protection PUT) and an unreadable/invalid
#        required-contexts.json artifact.
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --allow-unproduced-context) ALLOW_UNPRODUCED=1; shift ;;
    -h|--help) usage 0 ;;
...
# --- PLAN-031 Phase C: unproduced-context guard (the #481 class) -----------
# A required status-check context whose producer caller is absent from the
# target repo never reports, so arming protection pins every PR on "Expected —
# Waiting for status to be reported" forever. All four armed tiers set
# enforce_admins:true (only umbrella is false), so there is no --admin escape
# and warn-only would preserve the worst case while merely logging it.
#
# The tier's required contexts resolve to producers via required-contexts.json
# — DERIVED at canon-authoring time by install/generate-required-contexts.py
# from required-context-map.py, never hand-maintained (a committed
# context->caller table is the F1 failure mode; the drift gate in
# tests/test_unproduced_contexts.sh is what makes committing it acceptable).
# apply-standards.sh runs from a consumer checkout with no canon tree, so it
# fetches that ONE file via the existing fetch_canon/CI_TAG mechanism; a fetch
# failure REFUSES via fetch_canon's exit 3. Each producer is then read IN THE
# TARGET REPO via the gh contents API, copying the resolve_branch_declaration
# pattern (~:779): 404 means absent (a legitimate adoption gap), anything else
# means the target could not be read and the run REFUSES — a guard that cannot
# read reports "cannot verify", never "verified".

apply_fetch_required_contexts() {
  # $1 = dest tmpfile. Fetch failure exits 3 inside fetch_canon (fail-closed
  # input: a guard that silently skips when its input is unavailable is the
  # documented sync/check-standards-drift.sh defect).
  fetch_canon "required-contexts.json" > "$1"
  if ! jq -e 'type == "object"' "$1" >/dev/null 2>&1; then
    echo "apply-standards: FATAL required-contexts.json from canon @ ${CI_TAG} is not a JSON object — refusing to arm." >&2
    exit 4
  fi
}

apply_producer_present() {
  # $1 = consumer path (.github/workflows/<producer>); $2 = file for stderr
  # capture. Returns 0 present, 1 absent (404), 2 unverifiable (auth/network).
  if gh api "repos/${REPO_LABEL}/contents/$1" --jq '.content' >/dev/null 2>"$2"; then
    return 0
  elif grep -qiE '404|not found|no such file' "$2"; then
    return 1
  else
    return 2
  fi
...
apply_guard_unproduced_contexts() {
  # Fail-closed gate for --apply: every required context of $TIER must have
  # its producer caller present IN THE TARGET REPO. Called once from apply_run
  # after confirm and before apply_backup, so refusal (exit 4) mutates nothing
  # at all — not just "no branch-protection PUT" but no labels, settings, or
  # actions either. (It used to run inside apply_branch_protection, after three
  # surfaces had already been PUT; the hang was prevented but the run was not
  # atomic. Do not move it back.)
  local artifact rows nreq=0 nmissing=0 nunverifiable=0
  local missing_lines="" unverifiable_lines=""
  artifact=$(mktemp)
  APPLY_TMPFILES+=("$artifact")
  apply_fetch_required_contexts "$artifact"
  if ! jq -e --arg tier "$TIER" 'has($tier) and (.[$tier] | type == "array")' "$artifact" >/dev/null 2>&1; then
    echo "apply-standards: FATAL required-contexts.json has no required-context array for tier '$TIER' — refusing to arm." >&2
    exit 4
  fi
  # Integrity cross-check: the artifact is fetched over the same channel as
  # every other canon file (TLS + immutable CI_TAG, no hash pin — consistent
  # with all canon fetches; the repo's SHA pins cover third-party binaries
  # only). A skewed artifact that OMITS a required context would otherwise
  # pass every gate below and print "nothing to guard" — the exact silent-pass
  # shape this guard exists to end. So require every context in the tier's own
  # branch-protection template to be present in the artifact. Extra artifact
  # contexts are harmless (they get checked too — the fail-safe direction).
  # Residual MITM acceptance is documented, not eliminated: TLS plus immutable
  # tags bound it; nothing here removes it.
  local tplf tpl_ctx art_ctxs missing_tpl=""
  tplf="$(apply_canon_stripped "branch-protection-${TIER}.json")"
  # Shape-check the template the same way the artifact is shape-checked above:
  # a fetch that returned non-JSON (or wrong JSON) must refuse here, not yield
  # an empty context list that the loop below would read as "nothing to
  # cross-check". (A fetch that fails outright already exits 3 inside
  # fetch_canon.)
  if ! jq -e 'has("required_status_checks")' "$tplf" >/dev/null 2>&1; then
    echo "apply-standards: FATAL branch-protection-${TIER}.json from canon @ ${CI_TAG} is not a branch-protection template — refusing to arm." >&2
    exit 4
  fi
  art_ctxs="$(jq -r --arg tier "$TIER" '.[$tier][] | .context' "$artifact" 2>/dev/null)"
  while IFS= read -r tpl_ctx; do
    [ -n "$tpl_ctx" ] || continue
    if printf '%s\n' "$art_ctxs" | grep -qxF -- "$tpl_ctx"; then
      :
    else
      missing_tpl="${missing_tpl}  · '${tpl_ctx}' — required by branch-protection-${TIER}.json but ABSENT from required-contexts.json (stale or skewed artifact); cannot verify a producer.
"
    fi
  done < <(jq -r '.required_status_checks.contexts[]?' "$tplf" 2>/dev/null)
  if [ -n "$missing_tpl" ]; then
    echo "apply-standards: FATAL required-contexts.json @ ${CI_TAG} does not cover every required context of tier '$TIER' — refusing to arm rather than guarding a subset:" >&2
    printf '%s' "$missing_tpl" >&2
    exit 4
  fi
  # Rows are SOH-delimited, NOT tab-delimited: a `?` producer renders as an
  # EMPTY middle field, and TAB is IFS whitespace — POSIX `read` collapses
  # consecutive IFS-whitespace into one delimiter, silently shifting that
  # field into its neighbour (a `?` row would read as PRODUCED). SOH
  # (\001, non-whitespace) preserves empty fields; contexts never contain it.
  rows=$(jq -r --arg tier "$TIER" '.[$tier][] | [.context, (.producer // ""), (.cold_start_installed | tostring)] | join("\u0001")' "$artifact" 2>/dev/null) || {
    echo "apply-standards: FATAL could not parse required-contexts.json tier '$TIER' — refusing to arm." >&2; exit 4; }
  local perr prc ctx producer _cold
  perr=$(mktemp)
  APPLY_TMPFILES+=("$perr")
  while IFS=$'\001' read -r ctx producer _cold; do
    # _cold (cold_start_installed) is consumed but never read: the verdict keys
    # off the producer's presence IN THE TARGET plus the null-producer case,
    # never off the cold-start flag (a present producer beats its `!`, SB12).
    # The field must still be consumed positionally — dropping it would fold
    # the flag into $producer. Underscore-prefixed so the unused-variable
    # check stays silent deliberately rather than by accident.
    [ -n "$ctx" ] || continue
    nreq=$((nreq + 1))
    if [ -z "$producer" ]; then
      # `?`: canon ships NO producer at all — a canon defect, worse than `!`
      # (shipped but not cold-started), and never fixable by --add-surface.
      nmissing=$((nmissing + 1))
      missing_lines="${missing_lines}  · '${ctx}' — canon ships NO producer for this context (canon defect, not an adoption gap); do not arm this tier until canon ships one.
"
      continue
    fi
    apply_producer_present "$producer" "$perr"
    prc=$?
    if [ "$prc" -eq 0 ]; then
      :
    elif [ "$prc" -eq 1 ]; then
      nmissing=$((nmissing + 1))
      missing_lines="${missing_lines}  · '${ctx}' — producer ${producer} is NOT installed in ${REPO_LABEL}
    recovery: bash install.sh ${REPO_LABEL} --add-surface ${producer}
    (if ${producer} IS present, fix token scope instead — GitHub returns 404 for private repos the token cannot read, which looks exactly like 'absent'.)
"
    else
      nunverifiable=$((nunverifiable + 1))
      unverifiable_lines="${unverifiable_lines}  · '${ctx}' (producer ${producer})
"
    fi
  done <<< "$rows"
  if [ "$nunverifiable" -gt 0 ]; then
    # Unconditional: --allow-unproduced-context overrides a CONFIRMED absence,
    # never an unreadable target — absent and present are indistinguishable.
    echo "apply-standards: REFUSING to arm branch protection (tier=$TIER) on ${REPO_LABEL}: could not verify $nunverifiable producer(s) in the target repo (unreachable target or token without repo contents read)." >&2
    printf '%s' "$unverifiable_lines" >&2
    echo "  Fix the token scope and re-run. (--allow-unproduced-context does NOT override an unreadable target.)" >&2
    exit 4
  fi
  if [ "$nmissing" -eq 0 ]; then
    if [ "$nreq" -gt 0 ]; then
      echo "  branch-protection: producer guard OK — all $nreq required context(s) have producers in ${REPO_LABEL}"
    else
      echo "  branch-protection: producer guard OK — tier '$TIER' requires no contexts (nothing to guard)"
    fi
    return 0
  fi
  if [ "$ALLOW_UNPRODUCED" -eq 1 ]; then
    echo "  branch-protection: WARNING: arming $nmissing required context(s) with NO producer installed (--allow-unproduced-context) — PRs will wait on these checks forever until the producers are added:" >&2
    printf '%s' "$missing_lines" >&2
    return 0
  fi
  echo "apply-standards: REFUSING to arm branch protection (tier=$TIER) on ${REPO_LABEL}: $nmissing required context(s) have no producer in the target repo — arming would pin every PR on \"Expected — Waiting for status to be reported\" forever (enforce_admins=true, no --admin escape)." >&2
  printf '%s' "$missing_lines" >&2
  echo "  Override (NOT recommended): re-run with --allow-unproduced-context." >&2
  exit 4
}

apply_report_unproduced_local() {
  # Report-only twin for --check/--dry-run/--report with --tier: same canon
  # artifact, but producers resolve against the LOCAL checkout (non-mutating
  # modes never read remote consumer files, per the header contract), and
  # findings never fail the run — they say what --apply would refuse.
  local artifact rows nreq=0 nwould=0
  local would_lines="" ctx producer
  artifact=$(mktemp)
  APPLY_TMPFILES+=("$artifact")
  apply_fetch_required_contexts "$artifact"
  if ! jq -e --arg tier "$TIER" 'has($tier) and (.[$tier] | type == "array")' "$artifact" >/dev/null 2>&1; then
    echo "apply-standards: invalid --tier value: $TIER (no required-context array in required-contexts.json)" >&2
    exit 2
  fi
  rows=$(jq -r --arg tier "$TIER" '.[$tier][] | [.context, (.producer // "")] | join("\u0001")' "$artifact" 2>/dev/null) || {
    echo "apply-standards: FATAL could not parse required-contexts.json tier '$TIER'." >&2; exit 4; }
  while IFS=$'\001' read -r ctx producer; do
    [ -n "$ctx" ] || continue
    nreq=$((nreq + 1))
    if [ -z "$producer" ]; then
      nwould=$((nwould + 1))
      would_lines="${would_lines}  · '${ctx}' — canon ships NO producer for this context (canon defect); --apply would REFUSE.
"
    elif [ ! -f "$producer" ]; then
      nwould=$((nwould + 1))
      would_lines="${would_lines}  · '${ctx}' — producer ${producer} is MISSING locally
    recovery: bash install.sh <owner>/<repo> --add-surface ${producer}
"
    fi
  done <<< "$rows"
  # --report owns stdout (machine JSON); the human report goes to stderr there.
  local _o="/dev/stdout"
  [ "$MODE" = "report" ] && _o="/dev/stderr"
  if [ "$nwould" -gt 0 ]; then
    echo "apply-standards: [${MODE}] tier '$TIER' would REFUSE to arm ($nwould of $nreq required context(s) have no producer in this checkout):" >"$_o"
    printf '%s' "$would_lines" >"$_o"
  elif [ "$nreq" -gt 0 ]; then
    echo "apply-standards: [${MODE}] tier '$TIER': all $nreq required context producer(s) present locally." >"$_o"
  else
    echo "apply-standards: [${MODE}] tier '$TIER' requires no contexts (nothing to guard)." >"$_o"
  fi
  return 0
}
...
apply_branch_protection() {
  ...
  # NOTE: the unproduced-context guard does NOT live here. It runs once in
  # apply_run, after confirm and before apply_backup, so a refusal leaves the
  # target entirely untouched. It used to be called here, after labels,
  # repo-settings and actions had already been PUT — the hang was prevented
  # but the run was not atomic. Do not re-add the call here (double fetch,
  # and the atomicity guarantee would be lost).
  resolve_branch_declaration
...
apply_run() {
  ...
  if [ "$SKIP_BRANCH_PROTECTION" -eq 0 ]; then
    ...
    echo "       producer guard: REFUSE before any PUT when a required context's producer is absent from ${REPO_LABEL} (override: --allow-unproduced-context)"
  ...
  # The producer guard runs HERE — after confirm, before backup, before any
  # mutating surface. A refusal (exit 4) therefore leaves the target entirely
  # untouched, not partially mutated. Skipped when there is no PUT to guard.
  if [ "$SKIP_BRANCH_PROTECTION" -eq 1 ]; then
    : # apply_branch_protection will report the skip itself
  else
    apply_guard_unproduced_contexts
  fi
  apply_backup
...
case "$MODE" in
  check)
    emit_human
    if [ -n "$TIER" ]; then apply_report_unproduced_local; fi
