...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/vladm3105/aidoc-flow-ci/ci/v2.0.1/install/install.sh) \
  vladm3105/<consumer-repo> --visibility private

# Or override the tag explicitly:
CI_TAG=ci/v2.0.1 bash install.sh vladm3105/<consumer-repo> --visibility public
```
...
```bash
CI_TAG=ci/v2.0.1 bash install.sh acme/their-repo --visibility private \
  --codeowner acme-bot \
...
| **LiteLLM secrets** (ci/v2.0.0) | `LITELLM_BASE_URL` + `LITELLM_REVIEW_API_KEY` are required for the ai-review gate to connect to the LiteLLM proxy. `LITELLM_DOC_API_KEY` is required for doc-maintainer (optional). Set per-repo or at org level. | [`../docs/REVIEWER_APP_ONBOARDING.md`](../docs/REVIEWER_APP_ONBOARDING.md), [`docs/MIGRATION_v2.0.0.md`](MIGRATION_v2.0.0.md) |
