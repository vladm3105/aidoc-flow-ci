...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/vladm3105/aidoc-flow-ci/ci/v2.6.0/install/install.sh) \
  vladm3105/<consumer-repo> --visibility private

# Or override the tag explicitly:
CI_TAG=ci/v2.6.0 bash install.sh vladm3105/<consumer-repo> --visibility public
```
...
```bash
CI_TAG=ci/v2.6.0 bash install.sh acme/their-repo --visibility private \
  --codeowner acme-bot \
