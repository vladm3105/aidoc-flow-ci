...
whole, with no check that anything produces them. A cold-started repo armed this
way pins every PR on *"Expected — Waiting for status to be reported"* forever —
and all four armed tiers set `enforce_admins: true`, so there is no `--admin`
escape. This is the #481 class, previously documented as fixed for one instance;
the class stayed open for `call / verify` (needs `audit-trail.yml`) and
`call / gitleaks` (needs `secret-scan.yml`) at three tiers.

The guard resolves each required context to its producer via a new derived
artifact, `install/templates/required-contexts.json`
(`{tier: [{context, producer, cold_start_installed}]}`, generated by
`install/generate-required-contexts.py` from `required-context-map.py`), then
checks the **target repo itself** over the `gh` contents API — not what a cold
start would install, but what is actually there. Any required context whose
producer is absent (or unknown to canon, rendered as `"producer": null`, which
is a canon defect rather than a consumer gap) **refuses before anything is
mutated** — the guard runs in `apply_run` after confirm and before backup, not
inside the branch-protection step, so a refusal leaves the target untouched
rather than partially applied (exit 4). Absent-producer rows name each context,
its producer, and the exact recovery command
(`install.sh <repo> --add-surface .github/workflows/<producer>`); null-producer
rows print no recovery command because there is nothing to install. An unreadable
target also refuses, and `--allow-unproduced-context` does **not** override that
case. `--allow-unproduced-context` proceeds otherwise, but still names every gap
it is arming. `--skip-branch-protection` skips the guard (no PUT to guard), and
the non-mutating modes report what would refuse rather than exiting.

The artifact is committed but **derived, never hand-maintained** — a committed
table without a drift gate would re-introduce F1, so
`tests/test_unproduced_contexts.sh` (56 assertions) regenerates it and fails if
the committed copy drifted. The suite also proves the refusal is reachable (a
mutation flipping a genuinely-`true` producer fires the guard; flipping an
already-`false` one is refused by the harness rather than silently no-opping),
stubs `gh` by recording argv rather than just return values, and treats `?` as
at least as severe as `!`.

**Why MAJOR:** any `--apply` (interactive or not) whose tier has an unproduced
context now refuses (exit 4) where it previously armed unconditionally.
All-present repos, umbrella, and the check/dry-run/report modes are unaffected.
A warn-only default would preserve a permanent hang with no escape, so it was
rejected (PLAN-031 D1).
...
prediction is structurally incomplete: when a hook entry omits `stages:` and
`default_stages` is absent, the **hook repo's remote manifest** decides the
stage, and no parser of the consumer's config can see it.

Reproduced 2026-08-20 at `pre-commit` 4.5.1 and 4.6.0 (the pinned version; the
relevant output code is byte-identical in both), with a hook repo whose manifest
declares `stages: [pre-push]` and a config that says nothing about stages:

```console
guard:  pre-commit: 1 hook(s) selected at stage 'pre-commit'   exit 0  PASS
run:    (no hook output at all)                                exit 0  GREEN
```

A green required check that inspected nothing — the exact D11 shape the guard
exists to prevent, surviving inside the guard. The guard's own comment and a
test's assertion name both encoded the wrong premise, so the suite ratified it.

**Prediction cannot close that; observation can.** The `Run hooks` step now
captures `pre-commit`'s output and requires that at least one hook actually
executed, reporting the count it verified. `pre-commit` prints one dotted line
per executed hook and nothing for a hook the stage did not select.

**On BOTH surfaces.** All eight consumers and canon's own `self-pre-commit.yml`
call the `workflow_call` reusable, not the composite action, so an action-only
fix would have landed where nothing runs. `.github/workflows/pre-commit.yml` had
neither the pre-check nor the post-condition; it now carries the same body, and
a test asserts the two cannot drift. This is the second instance in one week of
canon shipping two surfaces per check — see the scanners entry below.
`REPO_STANDARDS.md` §14.1a had reserved this move for a founder call (FT-31);
resolved as `DECISIONS.md` CI-0039, which records why the reservation's premise
no longer held.

**No consumer changes behaviour until it re-pins.** A repo whose hooks genuinely
select nothing at its configured stage will red on re-pin — that check was never
inspecting anything.

Five scoping decisions, each of which would otherwise be a new defect, and each
measured against the real tool:

- **The padding is not a fixed width.** It is computed per run from the longest
  selected hook name, and for that hook's `(no files to check)` line it collapses
  to **three dots**. A first implementation matched four or more, so it MISSED
  the legitimate line while still matching the illegitimate one — a false red on
  a correct config, blaming stage selection. The count now anchors on the outcome
  vocabulary, never on a padding width.
- **`SKIP` is refused.** `SKIP=a,b` makes `pre-commit` print a `Skipped` line per
  named hook and exit 0 — measured as "2 hook(s) EXECUTED … satisfied" with
  nothing run. A required check does not let its environment choose which hooks
  run, so the run is invoked under `env -u SKIP`.
- **`--color=never` is pinned.** Colour wraps the outcome word in ANSI so the
  line no longer ends in `Passed`; measured with `PRE_COMMIT_COLOR=always`, every
  line was missed and a green run hard-failed.
- **All hooks reporting `(no files to check)` is also zero inspection**, and now
  reds with its own diagnosis (file patterns, not stage selection). Counting one
  such hook as executed is right — the stage selected it — but counting all of
  them as inspection is the same silent green by another route.
- **The post-condition fires only on `rc == 0`**, and no longer prints
  "satisfied" on a failing run, where it was never evaluated. A failing run keeps
  `pre-commit`'s own diagnosis.

`tests/test_actions.sh` drives the shipped step on both surfaces against a
`pre-commit` shim that records its argv and whether `SKIP` reached the child — an
argv-only stub cannot see that defense. **Fixtures are generated by running the
real `pre-commit` where it is available**, because hand-written padding is what
hid the three-dot case; the fallback literals are the measured shapes. Five
mutations were confirmed to red the suite: removing `env -u SKIP`, removing
`--color=never`, restoring the four-dot regex, removing the all-no-files check,
and drifting the reusable body from the action.

Rule: `REPO_STANDARDS.md` § "D11 is asserted on the RUN, not predicted from the
config", which also states the general form: a gate that predicts what a tool
will inspect is only as good as its model of that tool; where the tool reports
what it did inspect, the report is the gate.
