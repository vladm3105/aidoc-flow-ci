...
| **🔴 [#438](https://github.com/vladm3105/aidoc-flow-ci/issues/438) — C0 arms a context bootstrap does not install** | The only load-bearing item to survive PLAN-026's three review passes. Substituting `quick-gates` into the tier templates bricks a post-v3 cold start: `quick-gates.yml` is `auto_install: false`, only `pre-commit.yml` is `true`, and `branch-protection-*.json` is outside the FT-30 gate. Both ways out hang a repo's `main` in some scenario | Founder decision. #438 carries the evidence and a recommendation (flip `auto_install` with a `replaces`-aware bootstrap skip) |
