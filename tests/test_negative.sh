...
assert_fail "printf '%s' '[self-hosted, ci, ephemeral]' | jq -e . >/dev/null 2>&1" "unquoted array rejected (the heredoc-quote-strip bug)"
assert_ok   'printf %s '"'"'["self-hosted", "ci", "ephemeral"]'"'"' | jq -e "type==\"array\"" >/dev/null 2>&1' "valid JSON array accepted"
