...
if command -v actionlint >/dev/null 2>&1; then
  # templates use ${{ }} that reference caller-only context; -shellcheck= disables
  # the nested-shell lint (noisy on the run: blocks we already unit-test).
  out="$(actionlint -shellcheck= .github/workflows/*.yml install/templates/workflows/*.yml 2>&1)" \
