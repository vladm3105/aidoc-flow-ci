...
| `docs-sync` | [`.github/workflows/docs-sync.yml`](../.github/workflows/docs-sync.yml) | **Mechanical** post-merge doc fixer (CHANGELOG stub-entry + version propagation + cross-ref repair). Per IPLAN-0018. Direct-commits to main with `[skip ci]`. **The workspace's sole doc automation** since `doc-maintainer` was retired (`DECISIONS.md` CI-0040); the IPLAN-0025 supersession never completed and is withdrawn. | `push` to the consumer's integration branch (the caller template's filter is `${INTEGRATION_BRANCH}`, resolved at install time — PLAN-028 B5; canon's own reusable is unchanged) |
