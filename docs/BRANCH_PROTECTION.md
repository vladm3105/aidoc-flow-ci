...
```bash
CI_TAG=ci/v2.4.0 bash <(curl -fsSL https://raw.githubusercontent.com/vladm3105/aidoc-flow-ci/ci/v2.4.0/install/apply-standards.sh) \
  --repo <owner>/<repo> --tier <governance|product|ops|umbrella|bootstrap> --apply --yes
```

- **`CI_TAG=ci/v2.4.0`** pins which canon the script applies. Without it,
...
curl -fsSL https://raw.githubusercontent.com/vladm3105/aidoc-flow-ci/ci/v2.4.0/install/templates/branch-protection-<tier>.json \
