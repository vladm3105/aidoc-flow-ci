...
jobs:
  call:
    ...
    with:
      runner_labels: '"runner-self"'   # PRIVATE override; default is "ubuntu-latest"
      globs: |
        **/*.md
        !node_modules
```
...
jobs:
  codeql:
    uses: vladm3105/aidoc-flow-ci/.github/workflows/codeql.yml@ci/v1.0.0
    with:
...
jobs:
  internal:
    uses: vladm3105/aidoc-flow-ci/.github/workflows/links.yml@ci/v1.0.0
    with:
      mode: internal
      paths: docs
```
...
| Question | Use mode |
|---|---|
| "I want the canonical behavior with `runner-self` instead of `ubuntu-latest`" | Mode 1 (parameter override) |
