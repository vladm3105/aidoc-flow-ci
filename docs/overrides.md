...
jobs:
  call:
    uses: vladm3105/aidoc-flow-ci/.github/workflows/markdown-lint.yml@ci/v3.0.0
    with:
      runner_labels: '["self-hosted", "ci", "ephemeral"]'   # PRIVATE override; default is "ubuntu-latest"
      globs: |
        **/*.md
        !node_modules
```
...
jobs:
  internal:
    uses: vladm3105/aidoc-flow-ci/.github/workflows/links.yml@ci/v3.0.0
    with:
      mode: internal
      paths: docs
```
...
| Question | Use mode |
|---|---|
| "I want the canonical self-hosted `ci` / `ephemeral` pool instead of `ubuntu-latest`" | Mode 1 (parameter override) |
