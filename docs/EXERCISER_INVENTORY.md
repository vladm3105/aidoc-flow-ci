...
| `actions/markdownlint/action.yml` | `test_actions.sh` (composite contract, D5, D15, D20, D21, precondition guard, cli2 tool-parity), `test_lint.sh` (`run:` bodies extracted and shellchecked) | offline-test — **not self-run**: canon cannot call its own composite actions until the `ci/v3.0.0` tag exists (FT-21) |
| `actions/pre-commit/action.yml` | `test_actions.sh` (as above, plus the D11 hooks-selected guard), `test_lint.sh` | offline-test — same FT-21 constraint |
| `actions/links/action.yml` | `test_actions.sh` (as above, plus D38 musl build and the internal/external posture, D42), `test_lint.sh` | offline-test — same FT-21 constraint |
| `actions/dep-scan/action.yml` | `test_actions.sh` (D20 checksum, D24 `--no-call-analysis`, D12 zero-coverage exit 128, D14), `test_lint.sh` | offline-test — same FT-21 constraint |
| `actions/trivy-scan/action.yml` | `test_actions.sh` (D20, D25 scanner restriction, D14), `test_lint.sh` | offline-test — same FT-21 constraint |
...
| `install/set-litellm-secrets.sh` | `test_litellm_secrets.sh` (loopback refusal, pre-write proxy probe, keep-unless-`--overwrite`, fail-closed on an unreadable secret list, no mint for a secret it would keep, key never on argv) | offline-test |
