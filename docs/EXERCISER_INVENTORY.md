...
| `.github/workflows/scanners.yml` | `test_actions.sh` (D27 job-level fork guard, D3, D4, verdict step, drift guard), `test_lint.sh` (actionlint + composite-body shellcheck), `test_checknames.sh`, `test_required_contexts.sh` | offline-test — **not self-run**: same FT-21 constraint as `quick-gates.yml`, plus semgrep cannot install on the current runner image (#349) |
